...
class OCRPipeline:
    ...
    def _init_ocr(self):
        ...
        # Try a few signatures across Paddle + PaddleOCR versions
        attempts = (
            {"lang": self.lang, "use_angle_cls": False, "show_log": False},
            {"lang": self.lang, "use_angle_cls": False},
            {"lang": self.lang},                      # minimal
            {"lang": self.lang, "use_angle_cls": False, "use_gpu": False},  # some Windows builds need this
        )
        errors = []
        for kw in attempts:
            try:
    ...
    def _ocr_on_array(self, arr: np.ndarray) -> Tuple[List[str], List[Dict[str, Any]], Dict[str, Any]]:
        try:
            ...
            if not lines:
                # fallback to raw color if binarized failed
